...
def cal_finaltaxes(rate1_InterestSaving, rate2_InterestSaving, rate3_InterestSaving, tbrk1_InterestSaving, tbrk2_InterestSaving, rate_InterestBonds, rate_SalesOfStocks, rate_Lottery,
                    ...
                    rate_FinalOther,threshold_Turnover,
                    JML_PH_BRUTO_DISKONTO_SBI, JML_PH_BRUTO_DISKONTO_SBI_ADJUSTED, JML_PH_BRUTO_OBLIGASI, JML_PH_BRUTO_PENJUALAN_SAHAM, JML_PH_BRUTO_HADIAH_UNDIAN,
    ...
    savinginc = max(0, JML_PH_BRUTO_DISKONTO_SBI_ADJUSTED)    
        
    finaltax_InterestSaving = (rate1_InterestSaving * min(savinginc, tbrk1_InterestSaving) 
                                + rate2_InterestSaving * min(tbrk2_InterestSaving - tbrk1_InterestSaving, max(0., savinginc - tbrk1_InterestSaving))  
                                + rate3_InterestSaving * max(0, savinginc - tbrk2_InterestSaving))
    
    # finaltax_InterestSaving = 0.2*(savinginc-tbrk1_InterestSaving)
    # finaltax_InterestSaving = max(finaltax_InterestSaving,0)
    ...
    finaltax_Other = max(0, rate_FinalOther * (JML_PU-threshold_Turnover)) 
    ...
    return (finaltax_InterestSaving, finaltax_InterestBonds, finaltax_SalesOfStocks, finaltax_Lottery, 
            ...
            finaltax_Dividend, finaltax_WifeIncome, finaltax_Other, finaltax_Total)
...
def cal_Total_Income(JNS_SPT_NUM, JML_PH_NETO_DN_DR_USAHA,JML_PH_NETO_DN_DR_PEKERJAAN_ADJ,JML_PH_NETO_DN_LAINNYA,
                          JML_PH_NETO_LN, JML_PH_NETO, Calculated_Total_Income_1770, Calculated_Total_Income_1770S, Calculated_Total_Income_1770SS, Calculated_Total_Income):
    if (JNS_SPT_NUM == 0):
    ...
    Calculated_Total_Income = JML_PH_NETO_DN_DR_USAHA_NEW + JML_PH_NETO_DN_DR_PEKERJAAN_NEW + JML_PH_NETO_DN_LAINNYA_NEW + JML_PH_NETO_LN_NEW
    if (JNS_SPT_NUM == 0):    
...
@iterate_jit(nopython=True)
def calc_cti_behavior(rate1, rate2, rate3, rate4, rate5, rate6, 
                     ...
                     elasticity_pit_taxable_income_value, Calculated_Taxable_Income,
                     Calculated_Taxable_Income_Behavior):
    """
    ...
    if Calculated_Taxable_Income<=0:
        elasticity=0
    elif Calculated_Taxable_Income<elasticity_taxable_income_threshold0:
        elasticity=elasticity_taxable_income_value0
    elif Calculated_Taxable_Income<elasticity_taxable_income_threshold1:
    ...
    if Calculated_Taxable_Income<0:
        marg_rate=0
    elif Calculated_Taxable_Income<=tbrk1:
        marg_rate=rate1
    elif Calculated_Taxable_Income<=tbrk2:
        marg_rate=rate2
    elif Calculated_Taxable_Income<=tbrk3:
        marg_rate=rate3
    elif Calculated_Taxable_Income<=tbrk4:
        marg_rate=rate4
    else:        
        marg_rate=rate5
    ...
    elif Calculated_Taxable_Income<=tbrk1_curr_law:
        marg_rate_curr_law=rate1_curr_law
    elif Calculated_Taxable_Income<=tbrk2_curr_law:
        marg_rate_curr_law=rate2_curr_law
    elif Calculated_Taxable_Income<=tbrk3_curr_law:
        marg_rate_curr_law=rate3_curr_law
    elif Calculated_Taxable_Income<=tbrk4_curr_law:
    ...
    frac_change_cti = elasticity*(frac_change_net_of_pit_rate)  
    cti_behavior = Calculated_Taxable_Income*(1+frac_change_cti)
    Calculated_Taxable_Income_Behavior = cti_behavior
    return Calculated_Taxable_Income_Behavior


"Calculation for Income Tax Payable"
@iterate_jit(nopython=True)
def cal_pit(rate1, rate2, rate3, rate4, rate5, tbrk1, tbrk2, tbrk3, tbrk4, 
            JNS_SPT_NUM, Calculated_Taxable_Income_Behavior, pitax_1770SS, pitax_1770S, pitax_1770, op_tax):
    """
    ...
    taxinc = Calculated_Taxable_Income_Behavior
